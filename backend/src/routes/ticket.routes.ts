...
import { downloadBookingTickets, getBookingTickets, resendBookingTickets, validateTicketQrCode } from '../controllers/ticket.controller';

const router = express.Router();

router.post('/tickets/validate', verifyFirebaseToken, checkRole(['operator', 'admin']), validateTicketQrCode);
router.get('/tickets/:bookingId', verifyFirebaseToken, getBookingTickets);
